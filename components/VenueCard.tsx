...
export function VenueCard({ v }: { v: Venue }) {
  ...
  return (
    <>
      ...
      <Link href={`/venue/${v.id}`} className={`card block transition group relative cursor-pointer ${justCheckedIn ? 'ring-2 ring-brand shadow-lg shadow-brand/50' : 'hover:ring-1 hover:ring-brand'}`}>
        {justCheckedIn && (
      ...
      <div className="card p-0 -mt-px pt-4 rounded-t-none">
        <button
          onClick={(e) => {
            e.preventDefault();
